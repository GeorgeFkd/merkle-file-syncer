...
protected:
    ...
    std::optional<FileNode *> findNode(const std::string &relativePath, FileNode *root) const;
    int countFileNodes(FileNode *node) const;
    void collectAllFiles(const FileNode *node, const QString &path, QList<QString> &files) const;
